...
from .apis.fetch_autofill_data import get_file_from_drive_and_return_ocr_data, get_autofill_data
...
def get_profile(request):
    try:
        ...
        user_data['profile_photo'] = user_data['profile_photo'].replace( "uc?id=", "file/d/")
        user_data['profile_photo'] = user_data['profile_photo'].replace( "&export=download", "/view")
        user_data['aadhaar_card'] = user_data['aadhaar_card'].replace( "uc?id=", "file/d/")
        user_data['aadhaar_card'] = user_data['aadhaar_card'].replace( "&export=download", "/view")
        user_data['pan_card'] = user_data['pan_card'].replace( "uc?id=", "file/d/")
...
def auto_fill_extension(request):
    try:
        ...
        try:
            ...
            print('user_data:', user_data)
            user_data['aadhaar_card'] = get_file_from_drive_and_return_ocr_data(user_data['aadhaar_card'])
            user_data['pan_card'] = get_file_from_drive_and_return_ocr_data(user_data['pan_card'])
            print('user_data:', user_data)
